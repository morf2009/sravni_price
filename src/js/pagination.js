$(function () {
   ...
   /*         console.log('old: ' + JSON.stringify(oldParams));
        ...
        $.ajax({
            ...
            success: data => {
                ...
                for (var i = 0; i < data.items.length; i++) {
                    getCardHtml(data.items[i].image_url, data.items[i].name, data.items[i].min_price, data.items[i].shops_count, data.items[i].category_url, data.items[i].reviews_count, data.items[i].avg_rate);
                }
    ...
    function getCardHtml(image_url, name, min_price, shops_count, min_price_shop_url, reviews_count, avg_rate) {
